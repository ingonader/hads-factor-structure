...
### Compare Loadings of Caci's model

```{r}
res_mi_mlr_ext %>% 
  select(model, group, constraint, status, 
         contains("load_")) %>%
  left_join(
    res_mi_ordinal_ext %>% 
      select(model, group, constraint, status, 
             contains("load_")),
    suffix = c("_MLR", "_WLSMV"),
    by = c("model", "group", "constraint")
  ) %>%
  select(model, group, constraint, 
         contains("status"), contains("load_min_"), contains("load_max_"), 
         everything()) %>%
